            ...
            switch(message.command) {
                ...
                case "USERNOTICE":
                  if (msgid === "resub" || msgid === "sub") {
                    ...
                    var months = _.get(~~message.tags["msg-param-months"], 0);
                    var plan = _.get(message.tags["msg-param-sub-plan"], null);
                    var plan_name = _.get(message.tags["msg-param-sub-plan-name"], null);
                    var userstate = null;
                    if (msg) {
                      userstate = message.tags;
                      userstate['message-type'] = msgid;
                    }
                    this.emit("subscription", channel, username, months, msg, plan, plan_name, false, userstate);
                  } else if (msgid === "subgift") {
                    var months = _.get(~~message.tags["msg-param-months"], 0);
                    var plan = _.get(message.tags["msg-param-sub-plan"], null);
                    var plan_name = _.get(message.tags["msg-param-sub-plan-name"], null);
                    var userstate = null;
                    var username = _.get(message.tags["msg-param-recipient-user-name"], null);
                    var paid_for_by_username = _.get(message.tags["login"], null);
                    if (msg) {
                      userstate = message.tags;
                      userstate['message-type'] = msgid;
                    }
                    this.emit("subscription", channel, username, months, msg, plan, plan_name, paid_for_by_username, userstate);
                  } else if (msgid === "raid") {
                    var username = message.tags["display-name"] || message.tags["login"];
                    var raiders = _.get(message.tags["msg-param-viewerCount"], null);
                    var subscriber_status = message.tags.subscriber;
                    var userstate = message.tags;
                    this.emit("raid", channel, username, raiders, subscriber_status, userstate);
                  }
                  break;
